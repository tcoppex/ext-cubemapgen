...
bool CDXUTDialog::MsgProc( HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam )
{
        ...
        case WM_MOUSEWHEEL:
        {
            ...
            switch( uMsg )
            {
               ...
               case WM_MOUSEWHEEL:
                    ...
                    if ( ( mousePoint.x >= 0 && mousePoint.x <= m_width &&
                           mousePoint.y >= 0 && mousePoint.y <= m_height ) )
                    {
                       // mouse is over this dialog,
                       // don't allow message to continue processing
                       return true;
                    }
                 break;
            }
    ...
    //Return false to indicate the message should still be handled by the application (usually to move the camera).
    return false;
}
...
HRESULT CDXUTDialogResourceManager::CreateTexture( UINT iTexture )
{
   ...
   //create the texture from within the resource

   //load raw file data from resource
   resourceInfo = FindResource(NULL, MAKEINTRESOURCE(IDR_UITEXTURE), L"TEXTUREDDS"); 

   if(resourceInfo == NULL)
   {
      return E_FAIL; //DXTRACE_ERR( L"FindResource", hr );
   }

   resourceData = LoadResource(NULL, resourceInfo ); 
   rawFileData = (unsigned char *)LockResource( resourceData);
...
//--------------------------------------------------------------------------------------
BOOL CDXUTComboBox::ContainsPoint( POINT pt ) 
{
   RECT dropDownWithScroll = m_rcDropdown;
    ...
    switch( uMsg )
    {
        ...
        case WM_LBUTTONUP:
        {
            if( m_bPressed )
            {
                m_bPressed = false;
                ReleaseCapture();
                m_pDialog->SendEvent( EVENT_SLIDER_VALUE_COMMIT, true, this );
